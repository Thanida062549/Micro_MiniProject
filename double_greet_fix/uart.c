 ...
 *
 * สำคัญ: ต้องเช็ค "บิตเปิด interrupt" (TXEIE / TCIE) คู่กับ flag เสมอ
 *
 * เพราะหลังรีเซ็ตทุกครั้ง USART_SR มีค่าเริ่มต้น 0x00C0 คือ TXE = 1 และ TC = 1
 * อยู่แล้วตั้งแต่ยังไม่เคยส่งอะไรเลย ถ้าเช็คแต่ flag อย่างเดียว พอสั่งพิมพ์
 * ข้อความแรกหลังบูต สาขา TC จะทำงานทันทีตั้งแต่ไบต์แรก แล้วเคลียร์ uart_busy
 * ทั้งที่ยังส่งไม่จบ -> BSP_UART_Print ครั้งถัดไปจะไม่รอ แล้วเขียนทับบัฟเฟอร์
 * กลางคัน ทำให้ข้อความซ้ำ สลับลำดับ หรือขาดหาย
 *
 * อาการจึงโผล่เฉพาะ "ข้อความแรกหลังรีเซ็ต" เท่านั้น ไม่เกิดระหว่างเล่นเกม */
void USART2_IRQHandler(void)
{
    ...
    if ((USART2_CR1 & USART_CR1_TCIE) && (USART2_SR & USART_SR_TC)) {
        USART2_CR1 &= ~USART_CR1_TCIE;
